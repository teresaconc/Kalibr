...
boost::python::tuple findTarget1(aslam::cameras::GridDetector * gd,
                                 ...
                                 const image_t & image, bool depth) {
  aslam::cameras::GridCalibrationTargetObservation obs(gd->target());
  cv::Mat to;
  eigen2cv(image, to);
  bool success = gd->findTarget(to, stamp, obs,depth);
...
boost::python::tuple findTarget2(
    ...
    const Eigen::Matrix<boost::uint8_t, Eigen::Dynamic, Eigen::Dynamic> & image, bool depth) {
  return findTarget1(gd, aslam::Time(0, 0), image,depth);
}
...
boost::python::tuple findTargetNoTransformation1(aslam::cameras::GridDetector * gd,
  ...
  cv::Mat to;
  eigen2cv(image, to);
  bool success = gd->findTargetNoTransformation(to, stamp, obs, depth);
...
boost::python::tuple findTargetNoTransformation2(aslam::cameras::GridDetector * gd,
                                                 const image_t & image,bool depth) {
  return findTargetNoTransformation1(gd, aslam::Time(0, 0), image, depth);
}
...
boost::python::tuple imageGridPoint(
    ...
    size_t c) {
  Eigen::Vector2d p = Eigen::Vector2d::Zero();
  SM_INFO_STREAM("In Grid Calib calling imageGridPOint");
