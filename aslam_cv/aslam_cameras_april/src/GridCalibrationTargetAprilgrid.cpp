...
GridCalibrationTargetAprilgrid::GridCalibrationTargetAprilgrid(
    ...
    : GridCalibrationTargetBase(2 * tagRows, 2 * tagCols),  //4 points per tag
      ...
      _options(options),
      _tagCodes(AprilTags::tagCodes25h4) {
...
void GridCalibrationTargetAprilgrid::createGridPoints() {
  ...
  std::vector<std::array<float, 3> > tag_points(_cols/2*_rows/2);
  std::vector<std::array<float, 3>>::iterator it;
 SM_INFO_STREAM("enter create grid size "<< tag_points.size());
  
  it = tag_points.begin();
  tag_points.insert ( it ,   {5,25.5,22} );
  
  it = tag_points.begin();
  tag_points.insert ( it +2,   {80,10,44} );
  
  it = tag_points.begin();
  tag_points.insert ( it +4 ,   {269.7,10,44} );
  
  it = tag_points.begin();
  tag_points.insert ( it +6,   {366.7,25.5,22} );
  
  it = tag_points.begin();
  tag_points.insert ( it +8,   {27,52.5,22} );
  
  it = tag_points.begin();
  tag_points.insert ( it+10 ,   {152.85,19.5,88} );

  it = tag_points.begin();
  tag_points.insert ( it +12,   {344.7,52.5,22} );
  
  it = tag_points.begin();
  tag_points.insert ( it +14,   {5,79.5,22} );
  
  it = tag_points.begin();
  tag_points.insert ( it+16 ,   {80,73,44} );

  it = tag_points.begin();
  tag_points.insert ( it+18 ,   {269.7,73,44});
  
  it = tag_points.begin();
  tag_points.insert ( it +20,   {366.7,79.5,22});
 ...
 float size;
  ...
  for (unsigned r = 0; r < _rows; r++) {
      for (unsigned c = 0; c < _cols; c++)  {
        ...
        index=r * _cols + c;
       /// tag_index= index/std::pow(2,r) -(r%2)*_cols/std::pow(2,r);
        tag_index = index/2 -(r/2)*_cols/2 - (r%2)*_cols/2;
        //SM_INFO_STREAM(" index -> tag_index " <<  index<< " -> " << tag_index << " Tag " << tag_points.at(tag_index)[0] );

        if(tag_points.at(tag_index)[0]==0)
          continue;

        
        size= tag_points.at(tag_index)[2]*0.001;
        point(0) = tag_points.at(tag_index)[0]*0.001 + (c % 2)*size;
        point(1) = tag_points.at(tag_index)[1]*0.001 + (r % 2)*size;
        point(2) = 0.0;
        
        //SM_INFO_STREAM(" index " <<  index<< " -> " << point(0) << " " << point(1));

        //Change system of numbers!!!!!!!!!!
...
/// \brief extract the calibration target points from an image and write to an observation
 bool GridCalibrationTargetAprilgrid::computeObservation(
    ...
    std::vector<bool> &outCornerObserved, bool depth) const  {
 ...
 // SM_INFO_STREAM("detections" << detections.size());
 ...
 /* handle the case in which a tag is identified but not all tag
  ...
  int valid = _options.minTagsForValidObs;
  //did we find enough tags?
  if(depth)
    valid = 1; //Valid Obs??? 1, 2? 1 can have false positives....
  if (detections.size() < valid) {
    success = false;
  ...
  if (detections.size() > 1) {
    for (unsigned i = 0; i < detections.size() - 1; i++)
      if (detections[i].id == detections[i + 1].id) {
        ...
        cvStartWindowThread();
        SM_INFO_STREAM(" obsCode and code i:" << std::hex << detections[i].obsCode << " " << std::hex << detections[i].code << " i+1: " << std::hex << detections[i+1].obsCode << " " << std::hex << detections[i+1].code);
  ...
  for (unsigned int i = 0; i < detections.size(); i++) {
    ...
    //SM_INFO_STREAM("size: " << detections.size() << " tagId, pos and baseId " << tagId << " " << pos << " " <<baseId);
    ...
    for (int j = 0; j < 4; j++) {
      ...
      //SM_INFO_STREAM("tagId " << tagId << " pos  " <<pos << " baseId " << baseId <<" valid corner " << outCornerObserved[pIdx[j]] );
