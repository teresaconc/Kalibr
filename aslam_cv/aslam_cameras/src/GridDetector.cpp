...
bool GridDetector::findTarget(const cv::Mat & image,
                              GridCalibrationTargetObservation & outObservation,bool depth) const {
  return findTarget(image, aslam::Time(0, 0), outObservation,depth);
}
...
bool GridDetector::findTargetNoTransformation(const cv::Mat & image, const aslam::Time & stamp,
  ...
  success = _target->computeObservation(image, cornerPoints, validCorners,depth);
...
//if Success is true.. we have validCorners and updateImagePoint runs
    if (validCorners[i]){
      outObservation.updateImagePoint(i, cornerPoints.row(i).transpose());
     // SM_INFO_STREAM(cornerPoints.row(i));
    }
  }
  return success;
}
...
bool GridDetector::findTarget(const cv::Mat & image, const aslam::Time & stamp,
  ...
  bool success = findTargetNoTransformation(image, stamp, outObservation,depth);
...
bool GridDetector::findTargetNoTransformation(const cv::Mat & image,
                                              GridCalibrationTargetObservation & outObservation, bool depth) const {
  return findTargetNoTransformation(image, aslam::Time(0, 0), outObservation,depth);
}
