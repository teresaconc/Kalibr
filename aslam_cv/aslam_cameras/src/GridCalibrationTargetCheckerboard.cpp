...
bool GridCalibrationTargetCheckerboard::computeObservation(const cv::Mat & image,
           Eigen::MatrixXd & outImagePoints, std::vector<bool> &outCornerObserved, bool depth) const {
  SM_INFO_STREAM("computeObservation");
  ...
  if(depth)
    ...
    //cv::threshold( image, dst, 127, 255,1 );
  //cv::equalizeHist( image, dst ); 
  //cv::threshold( image, dst, 0, 255,1+cv::THRESH_OTSU ); //OTSU
  //cv::adaptiveThreshold(image, dst, 255, cv::ADAPTIVE_THRESH_GAUSSIAN_C,1, 5, 0); //or ADAPTIVE_THRESH_MEAN_C
  bool success = cv::findChessboardCorners(dst, patternSize, centers, flags);

  // do optional subpixel refinement
  if (_options.doSubpixelRefinement && success) {
    cv::cornerSubPix(
        dst, centers, cv::Size(_options.windowWidth, _options.windowWidth), cv::Size(-1, -1),
  ...
  //SM_INFO_STREAM("Success " << centers(0) << std::endl);
