...
bool PinholeProjection<DISTORTION_T>::keypointToEuclidean(
  ...
  kp[0] = (kp[0] - _cu) / _fu;
  kp[1] = (kp[1] - _cv) / _fv;
  //SM_INFO_STREAM(" CU, CV, FU, FV, KP");
  //SM_INFO_STREAM(_cu <<" "<< _cv << " "<< _fu <<" "<<  _fv <<" "<< kp[0] << " "<< kp[1]);
...
public:
  ...
  static std::vector<cv::Point2d> intersectCircles(double x1, double y1, double r1,
    ...
    //SM_INFO_STREAM("d: "<< d << " " <<x1 << " " << x2 << " " << y1 << " " << y2 << " " << r1 << " " << r2);
    if (d > r1 + r2) {
  ...
  static double medianOfVectorElements(std::vector<double> values)
  {
    ...
    //for (unsigned k=0; k< size ; k++)
     // SM_INFO_STREAM(values[k]);
    SM_INFO_STREAM("size " << size << " values "<< values[size / 2 - 1] << " " <<values[size / 2]) ;
    if (size%2 == 0)
...
bool PinholeProjection<DISTORTION_T>::initializeIntrinsics(const std::vector<GridCalibrationTargetObservation> &observations) {
  ...
  //SM_INFO_STREAM ("initializeIntrinsics");
  ...
  for (size_t i=0; i<nImages; ++i) {
    ...
    //changed skipImage for gridapril
    bool skipImage=true;
    ...
    for (size_t r=0; r<target.rows(); ++r) {
      ...
      //PROBLEM HERE, DOESN'T INITIALIZE FU AND FV
    ...
    for (size_t j=0; j<target.rows(); ++j)
    {
      for (size_t k=j+1; k<target.cols(); ++k)
      {
        ...
        //SM_INFO_STREAM("Ipts size " << ipts.size());
        if (ipts.size()<2)
          continue;
...
size_t PinholeProjection<DISTORTION_T>::computeReprojectionError(
  ...
  //SM_INFO_STREAM("computeReprojectionError");
...
bool PinholeProjection<DISTORTION_T>::estimateTransformation(
 ...
 // SM_INFO_STREAM(obs.getCornersTargetFrame(Ps));
  ...
  SM_INFO_STREAM("going to solvePnP " << cv::solvePnP(Ps, Ms, cv::Mat::eye(3, 3, CV_64F), distCoeffs, rvec, tvec));
