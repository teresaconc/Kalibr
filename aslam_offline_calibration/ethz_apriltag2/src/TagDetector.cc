...
namespace AprilTags {
  ...
  for (unsigned int qi = 0; qi < quads.size(); qi++ ) {
  ...
  #ifdef DEBUG_APRIL
        {
          if (v>threshold){
           cv::circle(image, cv::Point2f(irx, iry), 1, cv::Scalar(0,0,255,0), 2);
            cv::circle(image_clone, cv::Point2f(irx, iry), 1, cv::Scalar(0,0,255,0), 2);

          }
          else{
            cv::circle(image, cv::Point2f(irx, iry), 1, cv::Scalar(0,255,0,0), 2);

            cv::circle(image_clone, cv::Point2f(irx, iry), 1, cv::Scalar(0,255,0,0), 2);
          }
           cv::imshow("tags",image_clone);
            cv::waitKey(0);
    ...
    if ( !bad ) {    
      ...
      //thisTagFamily.printHammingDistances();
      //SM_INFO_STREAM("best id " << thisTagDetection.id );
