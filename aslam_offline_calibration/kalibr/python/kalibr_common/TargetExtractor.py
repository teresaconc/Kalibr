...
def multicoreExtractionWrapper(detector, taskq, resultq, clearImages, noTransformation,depth):    
    while 1:
        try:
        ...
        image = task[2]
        if noTransformation:
            success, obs = detector.findTargetNoTransformation(stamp, np.array(image),depth)
        else:
            success, obs = detector.findTarget(stamp, np.array(image),depth)
...
def extractCornersFromDataset(dataset, detector, multithreading=False, numProcesses=None, clearImages=True, noTransformation=False, depth=False):
    ...
    if multithreading:   
        ...
        try:      
            ...
            for pidx in range(0, numProcesses):
                ...
                p = multiprocessing.Process(target=multicoreExtractionWrapper, args=(detector_copy, taskq, resultq, clearImages, noTransformation,depth ))
                p.start()
                plist.append(p)
    ...
    #single threaded implementation
        ...
        for timestamp, image in dataset.readDataset():
            if noTransformation:
                success, observation = detector.findTargetNoTransformation(timestamp, np.array(image),depth)
            else:
                success, observation = detector.findTarget(timestamp, np.array(image),depth)
            if clearImages:
